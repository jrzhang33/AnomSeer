...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        ##########################
        if 0:
            actor_module.visual.requires_grad_(False) #####visual frozen
    ...
    ###### ruru
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_hidden_states_of_hint(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        numtext_list = [item['numtext'] for item in extra_info_list]
        enc = self.tokenizer(
           numtext_list, return_tensors='pt', padding="max_length", truncation=True,max_length = getattr(self.config, 'numtext_max_len', 512)
        ).to(torch.cuda.current_device()) 
 
        # ['responses', 'input_ids', 'attention_mask', 'position_ids']            padding=True, truncation=True, max_length=getattr(self.config, 'numtext_max_len', 512)
        data_hint = data.deepcopy()
        data_hint.batch['responses'] = enc['input_ids'] 
        data_hint.batch['input_ids'] = enc['input_ids']
        data_hint.batch['attention_mask'] = enc['attention_mask']
        data_hint.batch['position_ids'] = compute_position_id_with_mask(enc['attention_mask']) ##note
        # print("data_hint_input_id shape", data_hint.batch['input_ids'].shape)

        data_hint.non_tensor_batch.pop('multi_modal_inputs', None)


        # we should always recompute old_log_probs when it is HybridEngine
        data_hint.meta_info['micro_batch_size'] = self.config.rollout.log_prob_micro_batch_size_per_gpu
        data_hint.meta_info['max_token_len'] = self.config.rollout.log_prob_max_token_len_per_gpu
        data_hint.meta_info['use_dynamic_bsz'] = self.config.rollout.log_prob_use_dynamic_bsz
        data_hint.meta_info['temperature'] = self.config.rollout.temperature
        # perform recompute log_prob
        with self.ulysses_sharding_manager:
            data_hint = self.ulysses_sharding_manager.preprocess_data(data_hint)
            output, hint_hidden_state = self.actor.compute_log_prob(data=data_hint, use_out = True)
            output = DataProto.from_dict(tensors={'hint_hidden_states': hint_hidden_state,
                                                  'hint_attention_mask': enc['attention_mask']
                                                  },
...
#####################
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output, last_hidden_state = self.actor.compute_log_prob(data=data, use_out=True)
            output = DataProto.from_dict(tensors={'old_log_probs': output, 'last_hidden_states': last_hidden_state},
                                         meta_info={'temperature': self.config.rollout.temperature})
            output = self.ulysses_sharding_manager.postprocess_data(output)
